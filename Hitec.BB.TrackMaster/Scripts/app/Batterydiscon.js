...
function format(d) {
    ...
    if (data.BatteryCounter > 0) {
        var a = data.objIgnitionStatusReport;
        var tableString = '<table id="subTbl" class="table table-bordered" cellpadding="5" cellspacing="0" width="100%" ><thead ><tr"><th>Battery Disconnection start Time</th><th>Battery Disconnection End Time</th><th>Battery Disconnection start Location</th><th>Battery Disconnection End Location</th><th>Duration(Days-hh-mm-ss)</th></tr></thead>';
        a.forEach(function (element) {
            tableString = tableString +
            '<tr>' +
                //'<td>' + d.VehicleName + '</td>' +
                '<td>' + moment(element.BatteryOnTime).format("MMM D YYYY, hh:mm:ss a") + '</td>' +
                '<td>' + moment(element.BatteryOffTime).format("MMM D YYYY, hh:mm:ss a") + '</td>' +
...
$("#excelExport").click(function () {
    ...
    var url = baseUrl + "Reportsapi/GetBatterydisconReport";
...
function DownloadData(url, downloadType) {
    ...
    var reportName = 'Battery_disconnection_Report_' + sdt + '_TO_' + edt;
    var $bbid = '';
