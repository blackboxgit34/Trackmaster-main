...
function format(d) {
    ...
    if (data.ForwardCounter > 0) {
        var a = data.objIgnitionStatusReport;
        var tableString = '<table id="subTbl" class="table table-bordered" cellpadding="5" cellspacing="0" width="100%" ><thead ><tr"><th>Movement  start Date/Time</th><th>Movement Stop Date/Time</th><th>Movement  start Location</th><th>Movement  stop Location</th><th>Movement  Duration(Days-hh-mm-ss)</th></tr></thead>';
        a.forEach(function (element) {
            tableString = tableString +
            '<tr>' +
                //'<td>' + d.VehicleName + '</td>' +
                '<td>' + moment(element.ForwardOnTime).format("MMM D YYYY, hh:mm:ss a") + '</td>' +
                '<td>' + moment(element.ForwardOffTime).format("MMM D YYYY, hh:mm:ss a") + '</td>' +
...
$("#excelExport").click(function () {
    ...
    var url = baseUrl + "Reportsapi/GetForwardTravelReport";
...
function DownloadData(url, downloadType) {
    ...
    var reportName = 'Forward_Travel_Report_' + sdt + '_TO_' + edt;
    var $bbid = '';
