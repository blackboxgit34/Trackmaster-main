$(document).ready(function () {
    ...
    $('#dtGetVehInfo tbody').on('click', '.btn-submit', function (e) {
        ...
        var load = String($(this).closest('tr').find('.load').val());
        var Trailer = String($(this).closest('tr').find('.Trailer').val());
        ...
        var tyre = String($(this).closest('tr').find('.tyre').val());
        ...
        var tstatus = String($(this).closest('tr').find('.tstatus').val());
        var tremarks = String($(this).closest('tr').find('.tremarks').val());
        ...
        alertify.confirm(" Do you want to proceed?", function (e) {
            if (e) {
                ajaxResp(vehName, driverName, driverMobileNumber, vehOrder, mobNumForAlert, bbid, status, custid, driverLicenseNo, Type, Transport, group, load, Trailer, tyre, tstatus, tremarks)
                return true;
            } else {
    ...
    var ajaxResp = function (vehName, driverName, driverMobileNumber, vehOrder, mobNumForAlert, bbid, status, custid, driverLicenseNo, Type, Transport, group, load, Trailer, tyre, tstatus, tremarks) {
        $.ajax({
            dataType: "json",
            type: "GET",
            contentType: 'application/x-www-form-urlencoded; charset=utf-8',
            url: apiBase.apiurl + "AdminAPI/UpdateTrolleyBoxInfo",
            data: { vehName: vehName, driverName: driverName, driverMobileNumber: driverMobileNumber, vehOrder: vehOrder, mobNumForAlert: mobNumForAlert, bbid: bbid, status: status, custid: custid, driverLicenseNo: driverLicenseNo, Type: Type, Transport: Transport, group: group, Load: load, Trailer: Trailer, tyre: tyre, tstatus:tstatus, tremarks:tremarks },
            success: function (data) {
                toastr.success('successfully Done!');
            },
            error: function (error) {
        ...
        "columns": [
...

                {
...

                 {
                     ...
                     "orderable": false, "targets": 0,
                     "render": function (data, type, full, meta) {
                         return '<input type="text" value="' + full.load + '" class="form-control load" />';
                     }
                 },
                     {
                         ...
                         "render": function (data, type, full, meta) {
                          ...
                          },
                          {
                              ...
                              "orderable": false, "targets": 0,
                              "render": function (data, type, full, meta) {
                                  return '<input type="text" value="' + full.Trailer + '" class="form-control Trailer" />';
                              }
                          },
                           {
                           ...
                           },
                            {
                                ...
                                "orderable": false, "targets": 0,
                                "render": function (data, type, full, meta) {
                                    return '<span> <input type="text" value="' + full.tstatus + '" class="form-control tstatus" /></span>';
                                }
                            },
                            {
                                ...
                                "orderable": false, "targets": 0,
                                "render": function (data, type, full, meta) {
                                    return '<span> <input type="text" value=' + full.tyre + ' class="form-control tyre" /></span>';
                                }
                            },
                      {
...

                   {
                       ...
                       "orderable": false, "targets": 0,
                       "render": function (data, type, full, meta) {
                           return '<span> <input type="text" value="' + full.tremarks + '" class="form-control tremarks" /></span>';
                       }
                   },
