    ...
    $("#dt_basic_Master tr").each(function (rowIndex) {
        $(this).find("td").each(function (cellIndex) {
            ...
            if (cellIndexMapping[cellIndex]) {
                ...
                debugger;
                $.ajax({
                    url: apiBase.apiurl + "LiveStatus/Getmultifeaturestatus", // to get the right path to controller from TableRoutes of Asp.Net MVC
                    ...
                    success: function (data) {
                        ...
                        if (df.length == 5)
                        {
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(' + col + ')').text(df[0]);

                            if (df[1] == 0)
                            {
                                $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(13)').text('ON');

                            }
                            else
                            {
                                $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(13)').text('OFF');

                            }

                            if (df[2] == 0) {
                                $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(14)').text('Open');

                            }
                            else {
                                $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(14)').text('Close');

                            }
                            if (df[3] == 0) {
                                $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(15)').text('Open');

                            }
                            else {
                                $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(15)').text('Close');

                            }
                          
                           
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(16)').text(df[4]);
                        }
                        else
                        {
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(' + col + ')').text(df[0]);
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(13)').text('Not Working');
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(14)').text('Not Working');
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(15)').text('Not Working');
                            $('#dt_basic_Master tr:eq(' + rowIndex + ') td:eq(16)').text('Not Working');
                     ...
                     "render": function (data, type, full, meta) {
                         ...
                         if (full.ACTooltip == "AC Off" || full.ACTooltip == "AC On" || full.ACTooltip == "") {
                             ...
                             if (full.FuelActiveNTM == "1") {
                                 return '<span title="For Details go to add-on report"><b style="color:grey;"  ><a href="/AddOn/ac?vehName=' + full.VehicleName + '">' + full.AcActive + '</a></b></br><b style="color:grey;"><a href="/fuel/FuelGraph?bbid=' + full.BBID + '"><img src="../../Trackmaster_files/Fuel/Fuel.png" style="height:24px;"/></a> <br> <span  style="color:green;"><b>' + full.vehiclestatus + ' </b></span></b>' +
                                     '</span>';
                             }
                             else {
                                 return '<span title="For Details go to add-on report"><a href="/AddOn/ac?vehName=' + full.VehicleName + '">' + full.AcActive + '</a> <br><span  style="color:green;"><b> ' + full.vehiclestatus + '</b></span></b>' +
                                    '</span>';
                             }
                         }
