...
function format(d) {
    ...
    if (data.downCounter > 0) {
        var a = data.objIgnitionStatusReport;
        var tableString = '<table id="subTbl" class="table table-bordered" cellpadding="5" cellspacing="0" width="100%" ><thead ><tr"><th>Forklift Down start Time</th><th>Forklift Down End Time</th><th>Forklift Down start Location</th><th>Forklift Down End Location</th><th>Duration(Days-hh-mm-ss)</th></tr></thead>';
        a.forEach(function (element) {
            tableString = tableString +
            '<tr>' +
                //'<td>' + d.VehicleName + '</td>' +
                '<td>' + moment(element.downOnTime).format("MMM D YYYY, hh:mm:ss a") + '</td>' +
                '<td>' + moment(element.downOffTime).format("MMM D YYYY, hh:mm:ss a") + '</td>' +
...
$("#excelExport").click(function () {
    ...
    var url = baseUrl + "Reportsapi/GetLifdownReport";
...
function DownloadData(url, downloadType) {
    ...
    var reportName = 'Forklift _Down_Report_' + sdt + '_TO_' + edt;
    var $bbid = '';
