...
function format(d) {
    ...
    if (data.IgnitionOnOffCounter > 0) {
        ...
        var tableString = '<table id="subTbl" class="table table-bordered" cellpadding="5" cellspacing="0" width="100%" ><thead ><tr"><th>Engine On Time</th><th>Engine Off Time</th><th>Engine On Location</th><th>Engine Off Location</th><th>Duration (Days-hh-mm-ss)</th><th>Distance(KM)</th><th>Fuel Filling(ltrs)</th><th>Fuel Used(ltrs)</th><th>Mileage</tr></thead>';
            ...
            tableString = tableString +
                '<tr>' +
                    ...
                    '<td>' + element.TotalDistance + '</td>' +
                     '<td>' + element.TotalFilling + '</td>' +
                     
                     '<td>' + element.TotalConsumption + '</td>' +
                     '<td>' + element.TotalMileage + '</td>' +
                     //'<td>' + element.FuelTheft + '</td>' +
            '</tr>'
        });
...
$("#excelExport").click(function () {
    ...
    var url = baseUrl + "ReportsApi/GetEngineFuelReport";
...
function GetIgnitionReport(downloadType) {
    ...
    var url = apiBase.apiurl + "ReportsApi/GetEngineFuelReport";
    ...
    table = $('#dt_basic_Master').DataTable({
        ...
        "fnServerParams": function (param) {
         ...
         //   param.push({ "name": "bbid", "value": "" }, { "name": "beginDate", "value": $beginDate }, { "name": "endDate", "value": $endDate }, { "name": "CustId", "value": $custid }, { "name": "downloadType", "value": downloadType }, { "name": "reportName", "value": '' });
            param.push({ "name": "custid", "value": $custid }, { "name": "beginDate", "value": $beginDate }, { "name": "endDate", "value": $endDate });
        },
...
function DownloadData(url, downloadType) {
    ...
    var reportName = 'WorkingHours_Report_' + sdt + '_TO_' + edt;
    var $bbid = '';
