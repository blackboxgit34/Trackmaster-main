...
function format(d) {
    ...
    if (data.dataCount > 0) {
        ...
        var tableString = '<table id="subTbl" cellpadding="5" class="table table-hover" cellspacing="0" width="100%"   style = "outline-style: solid; outline-width: thin; outline-color: lightgray; "><thead><tr><th>Start Date</th><th>Start Time</th><th>Stop Date</th><th>Stop Time</th><th>Start Location</th><th>Stop Location</th><th>Equipment Idling Duration(hh:mm:ss)</th><th>Speed Limit</th><th>Max Speed</th><th>Distance</th><th>Towed/Travel Duration (hh:mm:ss)</th><th>Engine On Duration (hh:mm:ss)</th><th>Pump On Hour(h:mm:ss)</th><th>Pump On Location</th><th>Pump Idle Duration(hh:mm:ss)</th></thead>';
            ...
            tableString = tableString +
              ...
              '<td>' + element.Distance + '</td>' +
               '<td>' + element.Drivingduration + '</td>' +
                '<td>' + element.EngineOn + '</td>' +
                  '<td>' + element.PumpOn + '</td>' +
                   '<td>' + element.PumpOnLoc + '</td>' +
                    '<td>' + element.PumpIdle + '</td>' +
...
$("#excelExport").click(function () {
    ...
    var url = baseUrl + "AddOnAPI/CustomDailyReport";
