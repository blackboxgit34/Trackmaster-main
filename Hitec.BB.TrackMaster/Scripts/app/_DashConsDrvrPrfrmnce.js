...
$(document).ready(function () {
    ...
    $('#dt_basic_MasterCons tbody').on('click', 'td.details-control', function () {
        ...
        if (row.child.isShown()) {
            ...
            var inRange = percentage = (parseInt(row.data().InRangeSpeed) * 100) / row.data().Count;
            var OverSpeedLimitCount = (parseInt(row.data().OverSpeedLimitCount) * 100) / row.data().Count;

            var AccelerationCount = parseInt(row.data().AccelerationCount) > row.data().AccelerationTotal ? 0 : (parseInt(row.data().AccelerationCount) * 100) / row.data().AccelerationTotal;
            var HarshBrakingCount =parseInt(row.data().HarshBrakingCount) > row.data().HarshBrkingTotal ? 0: (parseInt(row.data().HarshBrakingCount) * 100) / row.data().HarshBrkingTotal;
          ...
          //  var outRange = parseInt(row.data().Count) - parseInt(row.data().InRangeSpeed);
            ...
            function drawChart() {
                var data = google.visualization.arrayToDataTable([
                  ['Task', 'Hours per Day'],
                  ['Average speed', inRange],
                  ['OverSpeed', OverSpeedLimitCount],
                  ['Acceleration', AccelerationCount],
                  ['Harsh Braking', HarshBrakingCount],
                ...
                var options = {
                    title: 'Vehicle Name: ' + row.data().VehicleName,
                    pieHole: 0.3,
...
function GetConsReport() {
    ...
    var url = baseUrl + "AdminAPI/ConsDrvrPrfrmnce";
    table = $('#dt_basic_MasterCons').DataTable({
        //"oLanguage": {
        ...
        "iDisplayLength": 3,
        "aLengthMenu": [[3,5, 10, 20, 25, 50], [3,5, 10, 20, 25, 50]],
        ...
        "columns": [
                ...
                // {
                //     "render": function (data, type, full, meta) {
                //        // var num = parseInt(full.HarshBrkingTotal) - parseInt(full.HarshBrakingCount);
                //         var percentage = parseInt(full.HarshBrakingCount) * 100 / full.HarshBrkingTotal;
                ...
                // {
                //     "render": function (data, type, full, meta) {
                //         var num = parseInt(full.AccelerationTotal) - parseInt(full.AccelerationCount);
                //         var percentage = parseInt(full.AccelerationCount) * 100 / full.AccelerationTotal;
