...
function format(d) {
    ...
    if (data.lidOpenCount > 0) {
        var a = data.doorLidData;
        var tableString = '<table id="subTbl" cellpadding="5" class="table table-hover" cellspacing="0" width="100%"   style = "outline-style: solid; outline-width: thin; outline-color: lightgray; "><thead><tr><th>Door Open Time</th><th>Door Closed Time</th><th>Duration (Day-hh:mm:ss)</th><th>Open Location</th><th>Close Location</th></tr></thead>';
...
$("#excelExport").click(function () {
    ...
    var url = baseUrl + "Reportsapi/GetDoorLidReport";
...
function GetDoorReport(downloadType) {
    
    //$custid = 6954; // to be deleted
   // $custid = 6954; // to be deleted

    deleteTable();
    var url = apiBase.apiurl + "ReportsApi/GetDoorLidReport";
...
function DownloadData(url, downloadType) {
    ...
    var reportName = 'Door_Report_' + sdt + '_TO_' + edt;
    var $bbid = '';
